    ...
    <div id="inner-wrap" class="inner-right">
        <div class="container row-fluid">
            ...
            <div class="row-fluid application">
                ...
                @if($page == 2)
                {{ Form::open(URL::to_route('app_create_entry', array ($instance->instance)), 'POST') }}
                {{ Form::token() }}
                @endif
                ...
                <div class="{{ $field->type->type }}" id="field_{{ $field->id }}">
                    {{ View::make('type.partials.'.$field->type->type)->with('field', $field)->with('disabled', $disabled)->with('instance', $instance)->with('page', $page)->with('end', $end) }}
                </div>
                @endforeach

                <!-- BACK BUTTON FOR THANK YOU PAGE -->
                @if($page == 4)
                <div <?php if(!empty($sort_buttons[0]->property)) echo 'style="background:'.$sort_buttons[0]->property.';"' ?> class="sort_items_4">{{ View::make('type.partials.'.$sort_buttons[0]->type->type)->with('field', $sort_buttons[0])->with('link', URL::to_route('app_show', array ($instance->instance))) }}</div>
                @endif

                <!-- ITEMS FOR VOTING -->
                @if(!empty($items))
                <!-- SORT BUTTONS -->
                <div <?php if(!empty($sort_buttons[0]->property)) echo 'style="background:'.$sort_buttons[0]->property.';"' ?> class="sort_items_{{$sort_buttons[0]->order}}">{{ View::make('type.partials.'.$sort_buttons[0]->type->type)->with('field', $sort_buttons[0])->with('link', URL::to_route('app_show', array ($instance->instance)).'?show=9') }}</div>
                <div <?php if(!empty($sort_buttons[1]->property)) echo 'style="background:'.$sort_buttons[1]->property.';"' ?> class="sort_items_{{$sort_buttons[1]->order}}">{{ View::make('type.partials.'.$sort_buttons[1]->type->type)->with('field', $sort_buttons[1])->with('link', URL::to_route('app_show', array ($instance->instance)).'?show=9&sort=votes') }}</div>
                ...
                <div class="span12 no-space">
                    ...
                    <div class="span4">
                        ...
                        <div class="img_container">
                            ...
                            <!-- VIDEO -->
                            <a href="{{ URL::to_route('app_item_detail', array($instance->instance, $item->id)) }}" class="fancybox-video fancybox.ajax" title="{{ $item->title }}" >
                            ...
                            @else
                            <!-- PHOTO -->
                            <a href="{{ URL::to_route('app_item_detail', array($instance->instance, $item->id)) }}" class="fancybox-image fancybox.ajax" title="{{ $item->title }}" >
                        ...
                        <div class="voting_detail">
                            ...
                            <!-- VOTE BUTTON -->
                            <div class="vote">
                                {{ Form::open(URL::to_route('app_vote', array ($instance->instance, $item->id))) }}
                                @if(isset($voted[$item->id]))
                                {{ View::make('type.partials.'.$item_buttons[0]->type->type)->with('field', $item_buttons[0])->with('disabled', array ('disabled' => 'disabled')) }}
                                @else
                                {{ View::make('type.partials.'.$item_buttons[0]->type->type)->with('field', $item_buttons[0])->with('disabled', $disabled) }}
                                @endif
                                {{ Form::close() }}
                            </div>

                            <!-- VIEW RESULTS BUTTON AND NUMBER OF VOTES-->
                            <div <?php if(!empty($item_buttons[1]->property)) echo 'style="background:'.$item_buttons[1]->property.';"' ?> class="check_results">{{ View::make('type.partials.'.$item_buttons[1]->type->type)->with('field', $item_buttons[1])->with('link', URL::to_route('app_item_result', array ($instance->instance, $item->id)))->with('options', array ('class' => 'item-result-detail fancybox.ajax')) }}</div>
                            ...
                            <!-- ENLARGE PHOTO/VIDEO -->
                            @if (File::extension($item->image) == 'mp4')
                            <div <?php if(!empty($item_buttons[3]->property)) echo 'style="background:'.$item_buttons[3]->property.';"' ?> class="enlarge">{{ View::make('type.partials.'.$item_buttons[3]->type->type)->with('field', $item_buttons[3])->with('link', URL::to($item->image))->with('options', array ('class' => 'fancybox-video', 'title' => $item->title)) }}</div>
                            @else
                            <div <?php if(!empty($item_buttons[3]->property)) echo 'style="background:'.$item_buttons[3]->property.';"' ?> class="enlarge">{{ View::make('type.partials.'.$item_buttons[3]->type->type)->with('field', $item_buttons[3])->with('link', URL::to($item->image))->with('options', array ('class' => 'fancybox-image', 'title' => $item->title)) }}</div>
                            @endif
                ...
                @if(!empty($show))
                <div <?php if(!empty($sort_buttons[3]->property)) echo 'style="background:'.$sort_buttons[3]->property.';"' ?> class="sort_items_{{$sort_buttons[3]->order}}">{{ View::make('type.partials.'.$sort_buttons[3]->type->type)->with('field', $sort_buttons[3])->with('link', URL::to_route('app_show', array ($instance->instance))) }}</div>
                @endif
                <div <?php if(!empty($sort_buttons[4]->property)) echo 'style="background:'.$sort_buttons[4]->property.';"' ?> class="sort_items_{{$sort_buttons[4]->order}}">{{ View::make('type.partials.'.$sort_buttons[4]->type->type)->with('field', $sort_buttons[4])->with('link', $invitelink)->with('options', array ('target' => '_top')) }}</div>
...
<div id="info-msg" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-body">
        <p>{{ empty($app_message) ? 'You have already voted for this item!' : $app_message }}</p>
    </div>
...
<div id="notification" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
        <h3 id="notification-title">{{ $instance->setting->title }}</h3>
    </div>
    <div class="modal-body">
        <p>
            @if(!empty($notification) && ($page == 3))
            {{ $notification }}
            @endif
        </p>
    </div>
