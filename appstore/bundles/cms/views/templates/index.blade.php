	            ...
	            @foreach($data  as $k => $v)
	            	<tr>
                      ...
                      @endif
	                	@endforeach
	                	<td>
	                		<a href="{{ URL::to_action('cms::templates@edit', array($v->id)) }}"><i class="icon-edit"></i></a>
	                		{{ Form::open(URL::to_action('cms::templates@destroy', array($v->id)), 'DELETE', array('style' => 'display:inline;')) }}
